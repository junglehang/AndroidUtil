package com.test.androidutil.sweetcamera.util;
...
import com.test.androidutil.R;
import com.test.androidutil.UtilApplication;
...
//    }
    ...
    public static Bitmap stringToBitmap(String text, int size, int color, Typeface font, int bgColor) {
        ...
        p.setTextSize(TypedValue.applyDimension(
                ...
                UtilApplication.CONTEXT.getResources().getDisplayMetrics()));

        float space =  UtilApplication.CONTEXT.getResources().getDimensionPixelSize(R.dimen.px_2);  //背景padding

        int textHeight = (int) getTextHeight(p);    //字的高度
        int pixLen = (int) (getFontHeight(p) + 2* space);//正方形边长

        int offsetY =   UtilApplication.CONTEXT.getResources().getDimensionPixelSize(R.dimen.px_2);    //y方向上的偏移  手动调整
    ...
    public static float getLineSpaceHeight(){
        return   UtilApplication.CONTEXT.getResources().getDimensionPixelSize(R.dimen.px_4);
    }
    ...
    public static float getCenterY(Paint paint){
        int offset =  UtilApplication.CONTEXT.getResources().getDimensionPixelSize(R.dimen.px_2);
