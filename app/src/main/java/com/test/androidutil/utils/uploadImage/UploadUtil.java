package com.test.androidutil.utils.uploadImage;
...
public class UploadUtil {
	...
	public void uploadFile(String filePath, String fileKey, String RequestURL,
						   Map<String, String> param, String name_str) {
	...
	public void uploadFile(final File file, final String fileKey,
		...
		new Thread(new Runnable() {  //启动线程上传文件
			public void run() {
...
//			conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded; charset=utf-8");
			...
			String imagefiledate    =    sDateFormat.format(new    java.util.Date());
			param.put("imageurl", imagefiledate);
...
//					dos.flush();
			...
			sb = new StringBuffer();
			/**
			 * 这里重点注意 name里面的为服务器端要key 只有这个key 才可以得到对应的文件
			 * filename是文件的名字，包含后名的 比如:abc.png
			 */
			sb.append(PREFIX).append(BOUNDARY).append(LINE_END);
//			sb.append("Content-Disposition:form-data; name=\"" + "imageurl"
//					+ "\"; filename=\"" +"456.png"+ file.getName() + "\"" + LINE_END);
			sb.append("Content-Disposition:form-data; name=\"" + name_str
					+ "\"; filename=\"" +imagefiledate+ file.getName() + "\"" + LINE_END);
			...
			if (res == 200) {
				 ...
				 */
				 if(!result.equals("")){
					 ...
					 Log.i("UploadUtil", "rusult= "+result);
				 }else{
					 Log.i("UploadUtil", "rusult= "+result);
				 }
				...
				try {
					switch (name_str) {
					case "work_img":
						int work_submit_state = jsonObject
								.getInt("work_submit_state");
						if (work_submit_state == 1) {
					...
					case "imageurl":
						String upstate = jsonObject.getString("upstate");
						if (upstate == "1") {
							Log.e(TAG, "result : " + result);
							sendMessage(UPLOAD_SUCCESS_CODE, "上传结果" + result);
							break;
